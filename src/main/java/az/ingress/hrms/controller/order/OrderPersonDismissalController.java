...
public class OrderPersonDismissalController {
    ...
    @Operation(
            ...
            description = "Returns all active order person dismissal records."
    )
    @ApiResponse(
            responseCode = "200",
            description = "Dismissal records retrieved successfully"
    )
    public ResponseEntity<List<OrderPersonDismissalResponse>> getAll() {
        return ResponseEntity.ok(service.getAll());
    }

    @GetMapping("/person/{personId}")
    @Operation(
            summary = "Get order person dismissals by Person ID",
            description = "Returns all dismissal records associated with a specific person."
    )
    @ApiResponses({
    ...
    public ResponseEntity<List<OrderPersonDismissalResponse>> getByPerson(
            @PathVariable
