...
public interface OrderPersonLeaveRepository extends JpaRepository<OrderPersonLeave, Integer>, JpaSpecificationExecutor<OrderPersonLeave> {
    ...
    List<OrderPersonLeave> findAllByOrderId(Integer orderId);

    @Query(value = "SELECT * FROM order_person_leave WHERE order_id = :orderId", nativeQuery = true)
    List<OrderPersonLeave> findAllByOrderIdWithDeleted(@Param("orderId") Integer orderId);


    List<OrderPersonLeave> findByOrderIdAndIsDeletedFalse(Integer orderId);

    Optional<OrderPersonLeave> findByOrderIdAndPersonId(Integer orderId, Integer personId);
}